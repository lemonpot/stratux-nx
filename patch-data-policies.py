...
new = '''\thttp.HandleFunc("/dataUsage/block", handleDataUsageBlock)\n\thttp.HandleFunc("/dataUsage/exempt", handleDataUsagePolicy)\n\thttp.HandleFunc("/dataUsage/policy", handleDataUsagePolicy)\n\thttp.HandleFunc("/dataUsage/settings", handleDataUsageSettings)'''
if old in s:
    s = s.replace(old, new, 1)
elif 'http.HandleFunc("/dataUsage/policy", handleDataUsagePolicy)' not in s:
    raise SystemExit('Could not register data usage device policy endpoint')
...
helper = marker + '''\nfunc dataUsagePolicyKey(ip, mac string) string {\n\tmac = strings.ToLower(strings.TrimSpace(mac))\n\tif mac != "" && mac != "00:00:00:00:00:00" {\n\t\treturn "mac:" + mac\n\t}\n\treturn "ip:" + strings.TrimSpace(ip)\n}\n\ntype dataUsageStoredPolicies struct {\n\tExempt map[string]bool   `json:"exempt"`\n\tLimits map[string]uint64 `json:"limitsMB"`\n}\n\nfunc loadDataUsagePoliciesLocked() {\n\tb, err := os.ReadFile(dataUsagePolicyPath)\n\tif err != nil {\n\t\treturn\n\t}\n\tvar stored dataUsageStoredPolicies\n\tif json.Unmarshal(b, &stored) == nil && (stored.Exempt != nil || stored.Limits != nil) {\n\t\tif stored.Exempt != nil {\n\t\t\tdataUsageExemptKeys = stored.Exempt\n\t\t}\n\t\tif stored.Limits != nil {\n\t\t\tdataUsageLimitKeys = stored.Limits\n\t\t}\n\t\treturn\n\t}\n\t// Migrate the original map[string]bool exemption format.\n\tvar legacy map[string]bool\n\tif json.Unmarshal(b, &legacy) == nil && legacy != nil {\n\t\tdataUsageExemptKeys = legacy\n\t}\n}\n\nfunc saveDataUsagePoliciesLocked() {\n\tstored := dataUsageStoredPolicies{Exempt: dataUsageExemptKeys, Limits: dataUsageLimitKeys}\n\tb, _ := json.MarshalIndent(stored, "", "  ")\n\t_ = os.WriteFile(dataUsagePolicyPath, b, 0644)\n}\n\nfunc dataUsageClientLimitMB(c *dataUsageClient, fallback uint64) uint64 {\n\tif c != nil && c.LimitMB > 0 {\n\t\treturn c.LimitMB\n\t}\n\treturn fallback\n}\n'''
...
new = '''\t\tc.Connected = true\n\t\tc.LastSeen = now.UTC().Format(time.RFC3339)\n\t\tpolicyKey := dataUsagePolicyKey(ip, c.MAC)\n\t\tipPolicyKey := dataUsagePolicyKey(ip, "")\n\t\tc.Exempt = dataUsageExemptKeys[policyKey] || dataUsageExemptKeys[ipPolicyKey]\n\t\tc.LimitMB = dataUsageLimitKeys[policyKey]\n\t\tif c.LimitMB == 0 {\n\t\t\tc.LimitMB = dataUsageLimitKeys[ipPolicyKey]\n\t\t}\n\t\tc.Blocked = blocked[ip]\n\t\tc.UploadBytes = upload[ip]'''
if old in s:
    s = s.replace(old, new, 1)
elif new not in s:
    raise SystemExit('Could not apply device policy during refresh')
...
new = '''\t\tclientLimitMB := dataUsageClientLimitMB(c, cfg.AutoBlockMB)\n\t\tclientLimitBytes := clientLimitMB * 1024 * 1024\n\t\tshouldBlock := c != nil && !c.Exempt && cfg.AutoBlockEnabled && clientLimitBytes > 0 && c.TotalBytes >= clientLimitBytes && !c.Blocked'''
if old in s:
    s = s.replace(old, new, 1)
elif new not in s:
    raise SystemExit('Could not apply device-specific auto block')

s = s.replace('\tautoBlockBytes := cfg.AutoBlockMB * 1024 * 1024\n', '', 1)

old = '''Message: fmt.Sprintf("Internet automatically blocked after exceeding %d MB", cfg.AutoBlockMB)'''
new = '''Message: fmt.Sprintf("Internet automatically paused after reaching the device limit of %d MB", clientLimitMB)'''
if old in s:
    s = s.replace(old, new, 1)
elif new not in s:
    raise SystemExit('Could not update device-specific auto block event')
...
if '$scope.toggleExempt' not in s and '$scope.applyDevicePolicy' not in s:
    if marker not in s:
...
if old in s:
    s = s.replace(old, new, 1)
elif 'data-session-reset' not in s and 'data-protection-panel' not in s:
...
if old in s:
    s = s.replace(old, new, 1)
elif 'data-policy-btn' not in s and 'data-policy-options' not in s:
