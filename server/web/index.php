...
<head>
  ...
  <style>
    :root{--bg:#f6f8fb;--card:#fff;--text:#1a1f36;--muted:#697386;--border:#e3e8ee;--primary:#635bff;--soft:#f0efff;--danger:#b4234d}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.5 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.top{height:58px;background:#fff;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;padding:0 28px}.brand{font-weight:700}.wrap{max-width:1100px;margin:0 auto;padding:36px 24px}.hero{margin-bottom:24px}.hero h1{font-size:28px;margin:0 0 6px}.hero p,.muted{color:var(--muted)}.card{background:var(--card);border:1px solid var(--border);border-radius:8px;margin:14px 0;overflow:hidden}.head{padding:16px 18px;border-bottom:1px solid var(--border)}.head h2{font-size:16px;margin:0}.body{padding:18px}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.device,.flight{border:1px solid var(--border);border-radius:7px;padding:14px}.device strong,.flight strong{display:block}.row{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.row.space{justify-content:space-between}.btn{display:inline-flex;align-items:center;justify-content:center;min-height:36px;padding:0 13px;border:1px solid var(--border);border-radius:6px;background:#fff;color:var(--text);font:600 13px inherit;text-decoration:none;cursor:pointer}.btn.primary{background:var(--primary);border-color:var(--primary);color:#fff}.btn.danger{color:var(--danger)}input{height:36px;border:1px solid var(--border);border-radius:6px;padding:0 10px;min-width:210px}.notice{padding:12px 14px;border-radius:6px;background:#fff3f6;color:var(--danger);margin-bottom:16px}.login{max-width:480px;margin:80px auto}.login .btn{width:100%;margin-top:10px}.pill{display:inline-block;padding:3px 7px;border-radius:99px;background:var(--soft);color:#5147e5;font-size:11px;font-weight:600}.empty{padding:22px;color:var(--muted);text-align:center}@media(max-width:700px){.grid{grid-template-columns:1fr}.top{padding:0 16px}.wrap{padding:24px 16px}.row.space{align-items:flex-start;flex-direction:column}input{width:100%}}
  </style>
</head>
<body>
<header class="top"><span class="brand">Stratux NX</span><?php if ($user): ?><a class="btn" href="/?action=logout">Sign out</a><?php endif ?></header>
<main class="wrap">
<?php if ($error): ?><div class="notice"><?=h($error)?></div><?php endif ?>
<?php if (!$user): ?>
  <section class="login card"><div class="head"><h2>Your Stratux NX flights</h2></div><div class="body"><p class="muted">Sign in to link one or more Stratux NX devices and see flights they have synchronized.</p><a class="btn primary" href="/?action=login-google">Continue with Google</a><a class="btn" href="/?action=login-apple">Continue with Apple</a></div></section>
<?php else: ?>
  <section class="hero"><h1>Flight history</h1><p><?=h($user['email'])?> · <?=count($devices)?> linked device<?=count($devices) === 1 ? '' : 's'?></p></section>
  <section class="card"><div class="head"><h2>Link a Stratux NX</h2></div><div class="body"><form class="row" method="post"><input type="hidden" name="csrf" value="<?=h(nx_csrf_token())?>"><input type="hidden" name="form_action" value="claim"><input name="code" placeholder="ABCD-2345" maxlength="9" required><button class="btn primary">Link device</button><span class="muted">Generate the one-time code from Flight Log on the device.</span></form></div></section>
